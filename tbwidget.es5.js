var _createTaboolaWidget = function() {
  ...
  var TBWidget = function() {
    ...
    function TBWidget(publisherid, apptype, apikey) {
      ...
      this.containers = [];
      this.url = "http://api.taboola.com/1.0/json/" + publisherid + "/recommendations.get?" + "app.type=" + apptype + "&app.apikey=" + apikey + "&source.id=%2Fdigiday-publishing-summit%2F" + // Should be added as parameter ?
        "&source.url=https%3A%2F%2Fblog.taboola.com%2Fdigiday-publishing-summit"; // Should be document.location.href
    }
    ...
    _createTaboolaWidget(TBWidget, [{
      ...
      value: function buildItem(item) {
        return "<div class='tb-item'>" + "<a href='" + item.url + "' target='_blank'>" + "<div class='tb-item-image' style='background-image:url(\"" + item.thumbnail + "\")'>" + "</div>" + "<div class='tb-item-label'>" + item.name + "</div>" + "<div class ='tb-item-branding'>" + item.branding + "</div>" + "<div class ='tb-item-category'>" + item.categories + "</div>" + "</div>" + "</a>" + "</div>";
      }
